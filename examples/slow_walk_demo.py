...
def save_csv(filename, rows):
    fieldnames = [
        "t", "phase",
        "x_ref", "y_ref", "x_real", "y_real", "z_real",
        "ex", "ey", "e_pos",
        "vx_real", "vx_ref", "vx_cmd_eff", "vy_cmd_eff", "wz_cmd_eff",
    ]
...
def main():
    ...
    parser.add_argument("--csv", type=str, default="slow_walk_metrics.csv")
    ...
    # Ganancias un poco más fuertes para regreso.
    ...
    print(
        f"\n[AVANZA + GIRO 180 + REGRESA] Robot: {args.robot_name} | "
        f"walk_cycle={args.cycle:.2f}s | turn_cycle={args.turn_cycle:.2f}s | "
        f"return_cycle={args.return_cycle:.2f}s | "
        f"vref={args.vref:.3f} m/s | return_vref={args.return_vref:.3f} m/s | "
    ...
    try:
        for step in range(n_steps):
            ...
            if t < turn_start:
                ...
                # Referencia visual: queremos volver hacia X menor.
                x_ref = x_after_turn - args.vref * after_turn_time
                y_ref = y_after_turn

                ramp_after = smoothstep(after_turn_time / 1.2)

                # vref_now se guarda negativo para que el CSV muestre que es regreso.
            ...
            if phase_name == "FORWARD":
                ...
                # Regreso con vx negativo.
                # Antes era positivo y por eso se iba hacia adelante.
                vx_cmd_eff = float(
            ...
            if step % int(1.0 / sim_dt) == 0:
                print(
                    f"t={t:4.1f}s | {phase_name:18s} | "
                    f"Swing:{'+'.join(active_swing) or '-':9s} | "
                    f"x_ref={x_ref:6.3f} y_ref={y_ref:6.3f} | "
                    f"x_real={x_real:6.3f} y_real={y_real:6.3f} | "
                    f"err={e_pos:5.3f} | "
